// ==UserScript==
// @name         Filters + Full VIDs 
// @namespace    http://tampermonkey.net/
// @version      1.8.0
// @description  Filters VIDs by name and color (excludes video cell authors) — now survives Madame’s single-page navigation
...
(function () {
  ...
  const worklistRootSelector =
    '.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation2.css-q5d465';
  ...
  function initButtons(container) {
    ...
    getOuterBoxes().forEach((box) => {
      ...
      box
        .querySelectorAll('.css-1ch8fxp[title*="Latest Retouched Author"]')
        .forEach((el) => {
      ...
      const colorEl = box.querySelector(
        '.MuiTypography-root.MuiTypography-body2.css-1pngg4p'
      );
    ...
    function filterRows() {
      ...
      getOuterBoxes().forEach((box) => {
        ...
        if (personalActive) {
          const spans = $$('.css-1ch8fxp[title*="Latest Retouched Author"]', box)
